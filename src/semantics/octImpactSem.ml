...
let rec set : update_mode -> Global.t -> ItvDom.Mem.t -> Proc.t -> PowOctLoc.t
  -> Cil.exp -> Dom.t -> Cil.location -> n_num:string ->  Dom.t
= fun mode global ptrmem pid lv_set e mem loc ~n_num ->
  let o = lookup mem in
  if try Cil.isIntegralType (Cil.typeOf e) with _ -> false then
    ...
    | Lval lval ->
        ItvSem.eval_lv pid lval ptrmem loc n_num
        |> PowOctLoc.of_locs
        |> (fun rv_set ->
      ...
      when (bop = PlusA && (Cil.i64_to_int i) = 1) || (bop = MinusA && (Cil.i64_to_int i) = -1) ->
        ...
        |> (fun rv_set ->
              if PowOctLoc.meet lv_set rv_set |> PowOctLoc.is_empty then
                set mode global ptrmem pid lv_set (Lval lval) mem loc ~n_num
              else
...
let alloc : update_mode -> Global.t -> ItvDom.Mem.t -> Proc.t -> PowOctLoc.t
  -> PowOctLoc.t -> Cil.exp -> Dom.t -> Cil.location -> n_num:string -> Dom.t
= fun mode global ptrmem pid lv_set ptrs e mem loc ~n_num->
  set mode global ptrmem pid ptrs e mem loc n_num

let rec prune : update_mode -> Global.t -> ItvDom.Mem.t -> Proc.t -> Cil.exp
  -> Dom.t -> Cil.location -> n_num : string -> Dom.t
= fun mode global ptrmem pid exp mem loc ~n_num ->
  let o = lookup mem in
  ...
  | Some (Cil.BinOp (bop, Cil.Lval x, Cil.Lval y, _))
    when bop = Lt || bop = Le || bop = Eq ->
      (x, y)
      |> BatTuple.Tuple2.mapn (fun lv -> ItvSem.eval_lv pid lv ptrmem loc n_num) 
  ...
  | Some (Cil.BinOp (bop, Cil.Lval x, Cil.BinOp (bop2, Cil.Const _, Cil.Lval y, _), t))
    when (bop = Lt || bop = Le || bop = Eq) && (bop2 = PlusA || bop2 = MinusA) ->
      prune mode global ptrmem pid (Cil.BinOp (bop, Cil.Lval x, Cil.Lval y, t)) mem loc n_num
  | Some (Cil.BinOp (bop, Cil.Lval x, Cil.Lval y, t))
    when bop = Gt || bop = Ge ->
      prune mode global ptrmem pid (Cil.BinOp (CilHelper.rev_binop bop, Cil.Lval y, Cil.Lval x, t)) mem loc n_num
  | _ -> mem

let model_realloc mode global node pid lvo exps ptrmem (mem, global) loc n_num =
  match lvo, exps with
    (Some l, _::size::_) ->
      let lv = ItvSem.eval_lv pid l ptrmem loc n_num |> PowOctLoc.of_locs in
      let ptrs = ItvSem.eval_array_alloc node size false ptrmem loc n_num |>
                 ItvDom.Val.allocsites_of_val |> PowOctLoc.of_sizes in
      alloc mode global ptrmem pid lv ptrs size mem loc n_num
  | _ -> mem

let model_calloc mode global node pid lvo exps ptrmem (mem, global) loc n_num =
  match lvo, exps with
    (Some l, size::_) ->
      let lv = ItvSem.eval_lv pid l ptrmem loc n_num |> PowOctLoc.of_locs in
      let ptrs = ItvSem.eval_array_alloc node size false ptrmem loc n_num |>
...
let model_strdup mode pid node lvo exps ptrmem (mem,global) loc n_num =
  match (lvo, exps) with
    (Some lv, str::_) ->
      let o = lookup mem in
      let lv_set = Allocsite.allocsite_of_node node |> OctLoc.of_size |> PowOctLoc.singleton in
      let rv_set = ItvSem.eval pid str ptrmem loc n_num |> ItvDom.Val.allocsites_of_val |>  PowOctLoc.of_sizes in
      let o =
            if can_strong_update_octloc mode global lv_set &&
               can_strong_update_octloc mode global rv_set
            then
...
let sparrow_arg mode pid exps ptrmem (mem,global) loc n_num =
  match exps with
    (Cil.Lval argc)::(Cil.Lval argv)::_ ->
      ...
      |> fun mem -> alloc mode global ptrmem pid lv argv_a (Cil.Lval argc) mem loc n_num
      |> forget mode global pid arg_a
  | _ -> mem

let model_strlen mode pid lvo exps ptrmem (mem, global) loc n_num =
  match lvo, exps with
  | (Some lv, (str::_)) ->
    let rv_set = ItvSem.eval pid str ptrmem loc n_num |> ItvDom.Val.allocsites_of_val |> PowOctLoc.of_sizes in
...
let handle_undefined_functions mode node pid (lvo,f,exps) ptrmem (mem,global) loc n_num  =
  match f.vname with
  | "sparrow_print" -> sparrow_print pid exps mem loc; mem
  | "sparrow_arg" -> sparrow_arg mode pid exps ptrmem (mem,global) loc n_num
  | "strlen" -> model_strlen mode pid lvo exps ptrmem (mem, global) loc n_num
  | "getenv" -> model_input mode pid lvo ptrmem (mem,global)
  | "strdup" -> model_strdup mode pid node lvo exps ptrmem (mem,global) loc n_num
  | "realloc" -> model_realloc mode global node pid lvo exps ptrmem (mem, global) loc n_num
  | "calloc" -> model_calloc mode global node pid lvo exps ptrmem (mem, global) loc n_num
  | _ ->
    (match lvo with
       None -> mem
     | Some lv ->
...
let binding : update_mode -> Global.t -> ItvDom.Mem.t -> Proc.t -> (Loc.t list) BatSet.t ->
  Cil.exp list -> Dom.t -> Cil.location -> n_num: string -> Dom.t
= fun mode global ptrmem pid paramset args mem loc ~n_num ->
  ...
  BatSet.fold (fun params mem ->
      ...
      List.fold_left2 (fun mem param arg ->
        ...
        set mode global ptrmem pid param arg mem loc n_num) mem params args 
  ) paramset mem
...
(*  prerr_endline (IntraCfg.Cmd.to_string cmd);*)
  ...
  | IntraCfg.Cmd.Cset (l, e, loc) ->
      let lv = ItvSem.eval_lv pid l ptrmem loc n_num |> PowOctLoc.of_locs in
      set mode global ptrmem pid lv e mem loc n_num
  ...
  | IntraCfg.Cmd.Calloc (l, IntraCfg.Cmd.Array e, is_static, loc) ->
      ...
      alloc mode global ptrmem pid lv ptrs e mem loc n_num
  ...
  | IntraCfg.Cmd.Cassume (e, loc) ->
      prune mode global ptrmem pid e mem loc n_num
  | IntraCfg.Cmd.Ccall (lvo, Cil.Lval (Cil.Var f, Cil.NoOffset), arg_exps, loc)
    when Global.is_undef f.vname global -> (* undefined library functions *)
      ...
      handle_undefined_functions mode node pid (lvo,f,arg_exps) ptrmem (mem,global) loc n_num
  | IntraCfg.Cmd.Ccall (lvo, f, arg_exps, loc) ->
      ...
      if PowProc.eq fs PowProc.bot then mem
        ...
        binding mode global ptrmem pid arg_lvars_set arg_exps mem loc n_num
  | IntraCfg.Cmd.Creturn (Some e, loc) ->
      let ret_locs = Dump.find (InterCfg.Node.get_pid node) global.dump |> PowOctLoc.of_locs in
      set mode global ptrmem pid ret_locs e mem loc n_num
