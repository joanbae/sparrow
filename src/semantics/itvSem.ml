...
let eval_const : Cil.constant -> Cil.location -> Val.t = fun cst loc ->
  ...
  | Cil.CInt64 (i64, _, _) ->
    ...
    Val.modify_footprints [%here] loc (Val.of_itv itv)
  | Cil.CStr s -> Val.modify_footprints [%here] loc (Val.of_itv Itv.top)
  | Cil.CWStr s -> Val.modify_footprints [%here] loc (Val.of_itv Itv.top)
  | Cil.CChr c -> Val.modify_footprints [%here] loc (Val.of_itv (Itv.of_int (int_of_char c)))
  ...
  | Cil.CReal (f, _, _) ->
   Val.modify_footprints [%here] loc (Val.of_itv (Itv.of_ints (int_of_float (floor f)) (int_of_float (ceil f))))
  (* Enum is not evaluated correctly in our analysis. *)
  | Cil.CEnum _ -> Val.modify_footprints [%here] loc (Val.of_itv Itv.top)
...
= fun spec u v loc ->
  if Val.eq v Val.bot then Val.bot else
    ...
    in
    Val.modify_footprints here loc (Val.join (Val.of_footprints fp) (Val.of_itv itv'))

let eval_bop : Spec.t -> Cil.binop -> Val.t -> Val.t -> Cil.location -> Val.t
  = fun spec b v1 v2 loc ->
    ...
    | Cil.PlusA ->
      ...
      Val.modify_footprints [%here] loc (Val.join v fp)
    | Cil.PlusPI
    | Cil.IndexPI ->
  ...
  | Cil.LOr ->
    ...
    Val.modify_footprints [%here] loc (Val.join v fp) 
  | Cil.Shiftlt ->
    if spec.Spec.unsound_bitwise then Val.modify_footprints [%here] loc v1
    else
...
and eval_lv ?(spec=Spec.empty) : Proc.t -> Cil.lval -> Mem.t -> Cil.location -> PowLoc.t * Footprints.t
  = fun pid lv mem loc ->
    let v, here =
      match fst lv with
      | Cil.Var vi ->
    ...
    in
    let powloc = PowLoc.remove Loc.null (resolve_offset spec pid v (snd lv) mem) in
    let fp = Footprints.join (Footprints.of_here here loc) (Val.footprints_of_val v) in
    (powloc, fp)
...
= fun pid e mem loc ->
  match e with
  | Cil.Const c -> Val.modify_footprints [%here] loc (eval_const c loc)
  | Cil.Lval l ->
    let powloc, fp = eval_lv ~spec pid l mem loc in
    Val.modify_footprints' [%here] fp loc (lookup powloc mem)
  | Cil.SizeOf t ->
    let sizeOf, here =
      Val.of_itv (try CilHelper.byteSizeOf t |> Itv.of_int with _ -> Itv.pos), [%here] in
    Val.modify_footprints here loc sizeOf
  | Cil.SizeOfE e ->
    let sizeOfE, here =
      Val.of_itv (try CilHelper.byteSizeOf (Cil.typeOf e) |> Itv.of_int with _ -> Itv.pos), [%here] in
    Val.modify_footprints here loc sizeOfE
  | Cil.SizeOfStr s ->
    let sizeOfStr, here = Val.of_itv (Itv.of_int (String.length s + 1)), [%here] in
    Val.modify_footprints here loc sizeOfStr
  | Cil.AlignOf t ->
    let alignOf, here = Val.of_itv (Itv.of_int (Cil.alignOf_int t)), [%here] in
    Val.modify_footprints here loc alignOf
  (* TODO: type information is required for precise semantics of AlignOfE.  *)
  | Cil.AlignOfE _ -> Val.modify_footprints [%here] loc (Val.of_itv Itv.top)
  | Cil.UnOp (u, e, _) ->
    let v, here= (eval_uop spec u (eval ~spec pid e mem loc) loc), [%here] in
    Val.modify_footprints here loc v
  | Cil.BinOp (b, e1, e2, _) ->
    let v1 = eval ~spec pid e1 mem loc in
    let v2 = eval ~spec pid e2 mem loc in
    Val.modify_footprints [%here] loc (eval_bop spec b v1 v2 loc) 
  | Cil.Question (e1, e2, e3, _) ->
    let i1 = Val.itv_of_val (eval ~spec pid e1 mem loc) in
    if Itv.is_bot i1 then
      Val.modify_footprints [%here] loc Val.bot
    else if Itv.eq (Itv.of_int 0) i1 then
      Val.modify_footprints [%here] loc (eval ~spec pid e3 mem loc)
    else if not (Itv.le (Itv.of_int 0) i1) then
      Val.modify_footprints [%here] loc (eval ~spec pid e2 mem loc)
    else
      let v, here = Val.join (eval ~spec pid e2 mem loc) (eval ~spec pid e3 mem loc), [%here] in
      Val.modify_footprints here loc v
  | Cil.CastE (t, e) ->
    let v, here = eval ~spec pid e mem loc, [%here] in
    (try Val.modify_footprints [%here] loc (Val.cast (Cil.typeOf e) t v)
     with _ -> Val.modify_footprints here loc v)
  | Cil.AddrOf l ->
    let powloc, fp = eval_lv ~spec pid l mem loc in
    Val.modify_footprints' [%here] fp loc (Val.of_pow_loc powloc)
  | Cil.AddrOfLabel _ ->
...
= fun spec pid exps mem loc ->
  List.map (fun e -> Val.modify_footprints [%here] loc (eval ~spec pid e mem loc)) exps
...
= fun node e is_static mem loc ->
  ...
  Val.modify_footprints [%here] loc (Val.join (Val.of_pow_loc pow_loc) (Val.of_array array))
...
= fun mode spec global pid cond mem ->
  ...
  | BinOp (op, Lval x, e, t)
    ...
    let x_lv, _ = eval_lv ~spec pid x mem (failwith "TODO") in
  ...
  | Lval x ->
    let x_lv = eval_lv ~spec pid x mem (failwith "TODO") in
    if PowLoc.cardinal x_lv = 1 then
      let x_v = lookup x_lv mem in
      let x_itv = Val.itv_of_val x_v in
...
let model_alloc_one mode spec pid lvo f (mem, global) =
  ...
  | Some lv ->
    ...
    let mem = update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) arr_val mem in
...
let model_realloc mode spec node (lvo, exps) (mem, global) =
  ...
  | Some lv ->
    begin
      match exps with
      | _::size::_ ->
        (update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (eval_array_alloc ~spec node size false mem) mem, global)
  ...
  | Some lv ->
    begin
      ...
      | n::size::_ ->
        ...
        (update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (eval_array_alloc ~spec node new_size false mem) mem, global)
      | _ -> raise (Failure "Error: arguments of realloc are not given")
    end
  | _ -> (mem,global)
  ...
  match exps with
    _::t ->
      List.fold_left (fun (mem, global) e ->
          match e with
            Cil.AddrOf lv ->
              let mem = update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) Val.itv_top mem in
              (mem, global)
...
let model_strdup mode spec node (lvo, exps) (mem, global) =
  ...
  | (Some lv, str::_) ->
    ...
    let mem = update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) v mem in
...
let model_input mode spec pid lvo (mem, global) =
  match lvo with
    Some lv ->
      ...
      let ext_loc = PowLoc.singleton (Loc.of_allocsite allocsite) in
      let mem = update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) ext_v mem in
      let mem = update mode spec global ext_loc Val.itv_top mem in
        (mem,global)
  | _ -> (mem,global)
...
let model_assign mode spec pid (lvo,exps) (mem, global) =
  match (lvo,exps) with
    (Some lv, e::_) ->
      (update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (eval ~spec pid e mem (failwith "TODO")) mem, global)
  | (_, _) -> (mem,global)
...
let model_strlen mode spec pid (lvo, exps) (mem, global) =
  ...
  | (Some lv, str::_) ->
    ...
    (update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) v mem, global)
  | _ -> (mem,global)
...
let rec model_fgets mode spec pid (lvo, exps) (mem, global) =
  ...
  | (_, Lval buf::size::_) | (_, StartOf buf::size::_) ->
    ...
    let buf_lv = eval_lv ~spec pid buf mem (failwith "TODO") in
...
let rec model_sprintf mode spec pid (lvo, exps) (mem, global) =
  ...
  | Lval buf::str::[] | StartOf buf::str::[] ->  (* format string *)
    ...
    |> (match lvo with Some lv -> update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (Val.of_itv null_pos) | _ -> id)
    |> (fun mem -> (mem, global))
  | CastE (_, buf)::str::[]
  ...
  | _ ->
    begin
      match lvo with
        Some lv -> (update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (Val.of_itv Itv.nat) mem, global)
      | _ -> (mem,global)
    end
...
let sparrow_arg mode spec pid exps (mem,global) =
  match exps with
    (Cil.Lval argc)::(Cil.Lval argv)::_ ->
      ...
      (update mode spec global (eval_lv ~spec pid argc mem (failwith "TODO")) (Val.of_itv Itv.pos) mem
      |> update mode spec global (eval_lv ~spec pid argv mem (failwith "TODO")) argv_v
...
let sparrow_opt mode spec pid exps (mem,global) =
  match exps with
    (Cil.Lval optind)::(Cil.Lval optarg)::_ ->
      let arg_a = Allocsite.allocsite_of_ext (Some "arg") in
      let arg_v = Val.of_array (ArrayBlk.input arg_a) in
      (update mode spec global (eval_lv ~spec pid optind mem (failwith "TODO")) (Val.of_itv Itv.nat) mem
      |> update mode spec global (eval_lv ~spec pid optarg mem (failwith "TODO")) arg_v
      |> update mode spec global (PowLoc.singleton (Loc.of_allocsite arg_a)) (Val.of_itv Itv.top), global)
  | _ -> (mem,global)
...
let model_memcpy mode spec pid (lvo, exps) (mem, global) =
  match (lvo, exps) with
    Some lv, dst::src::_ ->
      ...
      |> update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) dst_v
      |> (fun mem -> (mem, global))
  | _, _ -> (mem, global)
...
let model_getpwent mode spec node pid lvo f (mem,global) =
  match lvo, f.vtype with
    Some lv, Cil.TFun ((Cil.TPtr ((Cil.TComp (comp, _) as elem_t), _) as ptr_t), _, _, _) ->
      let struct_loc = eval_lv ~spec pid lv mem (failwith "TODO") in
...
let rec model_strcpy mode spec node pid es (mem, global) =
  ...
  | (Lval dst)::(StartOf src)::[] | (StartOf dst)::(Lval src)::[] ->
      let src_arr = Val.array_of_val (lookup (eval_lv ~spec pid src mem (failwith "TODO")) mem) in
      let dst_arr = Val.array_of_val (lookup (eval_lv ~spec pid dst mem (failwith "TODO")) mem) in
      let np = ArrayBlk.nullof src_arr in
      let newv = Val.of_array (ArrayBlk.set_null_pos dst_arr np) in
      let mem = mem |> update mode spec global (eval_lv ~spec pid dst mem (failwith "TODO")) newv in
      (mem, global)
  | _ -> (mem, global)
...
let rec model_strncpy mode spec node pid es (mem, global) =
  ...
  | (StartOf dst)::_::size::_ ->
      let arr = Val.array_of_val (lookup (eval_lv ~spec pid dst mem (failwith "TODO")) mem) in
...
let rec model_strchr mode spec node pid (lvo, exps) (mem, global) =
  ...
  | Some lv, (Lval str)::_ | Some lv, (StartOf str)::_ ->
      let str_arr = Val.array_of_val (lookup (eval_lv ~spec pid str mem (failwith "TODO")) mem) in
      let np = ArrayBlk.nullof str_arr in
      let newv = Val.of_array (ArrayBlk.plus_offset str_arr np) in
      let mem = mem |> update mode spec global (eval_lv ~spec pid lv mem (failwith "TODO")) newv in
      (mem, global)
  | _, _ -> (mem, global)
...
= fun mode spec node (mem, global) ->
  ...
  | IntraCfg.Cmd.Calloc (l, IntraCfg.Cmd.Array e, is_static, loc) ->
    (update mode spec global (eval_lv ~spec pid l mem (failwith "TODO")) (eval_array_alloc ~spec node e is_static mem) mem, global)
  | IntraCfg.Cmd.Calloc (l, IntraCfg.Cmd.Struct s, is_static, loc) ->
    let lv = eval_lv ~spec pid l mem (failwith "TODO") in
  ...
  | IntraCfg.Cmd.Csalloc (l, s, loc) ->
    ...
    in
    mem
    |> update mode spec global (eval_lv ~spec pid l mem (failwith "TODO")) (eval_string_alloc node s mem)
  ...
  | IntraCfg.Cmd.Cfalloc (l, fd, _) ->
    ...
    (update mode spec global (eval_lv ~spec pid l mem (failwith "TODO")) clos mem, global)
      ...
      let dump =
         ...
         | Some lv ->
           ...
           PowProc.fold (fun f d ->
              Dump.weak_add f (eval_lv ~spec pid lv mem (failwith "TODO"))
           ) fs global.dump in
  ...
  | IntraCfg.Cmd.Cskip when InterCfg.is_returnnode node global.icfg ->
    ...
    (match InterCfg.cmdof global.icfg callnode with
       IntraCfg.Cmd.Ccall (Some lv, f, _, _) ->
        ...
        update Weak spec global (eval_lv ~spec pid lv mem (failwith "TODO")) (lookup retvar_set mem) mem
     | _ -> mem)
    |> (fun mem -> (mem, global))
