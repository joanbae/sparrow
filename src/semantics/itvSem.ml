...
= fun spec pid v os mem (fp, loc) ~exp ~n_num ->
  ...
  | Cil.NoOffset ->
    ...
    (powloc, FP.join fp (FP.of_here here loc exp n_num))
  | Cil.Field (f, os') ->
    ...
    in
    resolve_offset spec pid v os' mem (Footprints.join fp (FP.of_here [%here] loc exp n_num), loc) exp n_num
  | Cil.Index (e, os') ->
    ...
    let fp = Footprints.join fp (FP.of_here [%here] loc exp n_num) in
    resolve_offset spec pid (ArrayBlk.pow_loc_of_array arr |> Val.of_pow_loc)  os' mem (fp, loc) exp n_num
...
and eval_lv_with_footprint ?(spec=Spec.empty) : Proc.t -> Cil.lval -> Mem.t -> Cil.location -> string -> PowLoc.t * Footprints.t
  = fun pid lv mem loc n_num ->
    ...
    in
    let powloc, fp = resolve_offset spec pid v (snd lv) mem (Footprints.of_here here loc exp n_num, loc) exp n_num in
