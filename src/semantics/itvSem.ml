...
let update : update_mode -> Spec.t -> Global.t -> PowLoc.t -> Val.t -> Mem.t -> Mem.t
  = fun mode spec global locs v mem ->
    let check_v = (** give priority when value gets updated **)
      ...
      else Val.modify_priority v 0  in
...
let model_alloc_one mode spec pid (lvo, exps) f (mem, global) loc n_info =
  match lvo with
    None -> (mem, global)
  | Some lv ->
    let s_exp = ExpArg.Fun (f.vname, exps) in
    ...
    let arr_val, here = Val.of_array (ArrayBlk.make allocsite Itv.zero size Itv.one Itv.nat), [%here] in
    let arr_val = Val.modify_footprints here loc s_exp n_info arr_val in
