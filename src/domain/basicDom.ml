  ...
  let rec compare x y =
    ...
    | Field (l1, f1, t1), Field (l2, f2, t2) ->
      ...
      if c = 0 then
        ...
        if c = 0 then Pervasives.compare (Cil.typeSig t1) (Cil.typeSig t2) else c
      else c
...
struct
  ...
  let priority x =
    let null = Loc.GVar ("NULL", Cil.voidPtrType) in
    let helper_pri x = if cardinal x >=2 then 3 else 0 in
    if is_empty x then 4 else
    if cardinal x >=2 then
